...
## 2026-09-25 (round 99) — `::part()` fell into the generic "unmodelled pseudo-element matches nothing" bucket alongside `::before`/`::after`, but unlike those it targets a REAL element that genuinely exists — confirmed live on developer.mozilla.org's own language-switcher button, inside a declarative shadow root (engine#192)
